...
const OptionWheel = ({
  ...
  const startLoop = useCallback(() => {
    // LOCAL PATCH (not upstream): this cancelled and rescheduled the frame on every
    // call, and reset the clock with it. A mouse wheel fires a burst of events between
    // two frames, so each one pushed lastRef forward; by the time a frame actually ran,
    // dt was near zero and k = 1 - exp(-dt/tau) collapsed with it, so the wheel barely
    // advanced that frame and then lurched. The loop already re-arms itself while it is
    // unsettled, so an in-flight frame is simply left alone and dt stays continuous.
  ...
  useEffect(() => {
    ...
    const onWheel = e => {
      ...
      // LOCAL PATCH (not upstream): this called preventDefault() unconditionally, so once
      // a non-looping wheel hit its first or last option it kept swallowing scroll and the
      // page could not be scrolled past it at all. Only consume the event while the wheel
      // can still act on it; otherwise let the scroll through to the page.
      // LOCAL PATCH: clamp how far a single burst of wheel events can push the target
      // ahead of where the animation currently is, so the easing never has more than
      // about one option of backlog to chase.
      const drift = targetRef.current - posRef.current;
      const damped = Math.abs(drift) > 1 && Math.sign(drift) === Math.sign(step) ? step * 0.25 : step;
      const next = targetRef.current + damped;
      const atEnd = !cfg.loop && ((step > 0 && targetRef.current >= cfg.count - 1) || (step < 0 && targetRef.current <= 0));
      if (atEnd) return;
      e.preventDefault();
