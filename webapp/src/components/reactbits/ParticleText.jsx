...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      // LOCAL PATCH (not upstream): `gatherOnMount={false}` starts the word already
      // assembled instead of flying together on load, for callers that want the text to
      // simply be there and only respond to the pointer. Reduced motion takes the same
      // path, since the assembling animation is exactly what it asks us to skip.
      if (reducedMotion || !gatherOnMount) {
        particles.forEach(particle => {
